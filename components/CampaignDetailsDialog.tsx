...
export default function CampaignDetailsDialog({
  ...
  // Load detailed campaign data when dialog opens
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.status && data.message) {
          ...
          // Update campaign data with detailed info
          ...
          // Initialize recipient statuses based on detailed data
          if (detailedCampaign.recipients) {
            const statuses = new Array(detailedCampaign.recipients.length);
            
            detailedCampaign.recipients.forEach((recipient: any, index: number) => {
              // Use recipient's individual status if available
              if (recipient.status) {
                statuses[index] = recipient.status;
              } else {
                // Fallback to campaign-level logic
                if (detailedCampaign.status === 'completed') {
  ...
  // Update campaign data when prop changes and load details
  useEffect(() => {
    if (campaign && open) {
  ...
  const handleCampaignProgress = useCallback((data: any) => {
    ...
    if (data.lastRecipient && data.lastMessageStatus && campaignData) {
      const recipientIndex = campaignData.recipients.findIndex(r => 
        r.name === data.lastRecipient || r.phone === data.lastRecipient
      );
      if (recipientIndex !== -1) {
  ...
  const handleCampaignComplete = useCallback((data: any) => {
    ...
    // Final status update for all recipients
    setRecipientStatuses(prev => {
      const newStatuses = [...prev];
      // Update based on final counts
          ...
          {!isLoadingDetails && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {/* Additional controls for processing campaigns */}
